...
if ($has_loai_huynh) {
    ...
    $sql_hdt = "SELECT t.id, t.ho_ten, t.ten_thanh, t.ngay_sinh, t.gioi_tinh, t.ngay_rua_toi, t.ngay_them_suc, t.ngay_gia_nhap, t.so_dien_thoai, t.email, t.dia_chi, t.ho_ten_bo, t.ho_ten_me, t.sdt_lien_lac, t.email_phu_huynh, t.ghi_chu, ct.vai_tro, c.ten_lop, c.nganh, t.loai_huynh, ct.class_id
    ...
    $sql_hdt = "SELECT t.id, t.ho_ten, t.ten_thanh, t.ngay_sinh, t.gioi_tinh, t.ngay_rua_toi, t.ngay_them_suc, t.ngay_gia_nhap, t.so_dien_thoai, t.email, t.dia_chi, t.ho_ten_bo, t.ho_ten_me, t.sdt_lien_lac, t.email_phu_huynh, t.ghi_chu, ct.vai_tro, c.ten_lop, c.nganh, 'Huynh Trưởng' as loai_huynh, ct.class_id
                FROM class_teachers ct 
                JOIN teachers t ON ct.teacher_id = t.id 
                JOIN classes c ON ct.class_id = c.id 
                WHERE 1=1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_hdt'])) {
    ...
    if ($has_loai_huynh) {
        $sql_teacher = "INSERT INTO teachers (ho_ten, ten_thanh, ngay_sinh, gioi_tinh, ngay_rua_toi, ngay_them_suc, ngay_gia_nhap, so_dien_thoai, email, dia_chi, ho_ten_bo, ho_ten_me, sdt_lien_lac, email_phu_huynh, ghi_chu, loai_huynh) 
                        VALUES ('$ho_ten', '$ten_thanh', '$ngay_sinh', '$gioi_tinh', '$ngay_rua_toi', '$ngay_them_suc', '$ngay_gia_nhap', '$so_dien_thoai', '$email', '$dia_chi', '$ho_ten_bo', '$ho_ten_me', '$sdt_lien_lac', '$email_phu_huynh', '$ghi_chu', '$loai_huynh')";
    } else {
        // Nếu chưa có các trường mới, chỉ thêm các trường cơ bản
        $sql_teacher = "INSERT INTO teachers (ho_ten, ten_thanh, ngay_sinh, so_dien_thoai, email, dia_chi, ngay_gia_nhap) 
                        VALUES ('$ho_ten', '$ten_thanh', '$ngay_sinh', '$so_dien_thoai', '$email', '$dia_chi', '$ngay_gia_nhap')";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_hdt'])) {
    ...
    if ($has_loai_huynh) {
        $sql_teacher = "UPDATE teachers SET ho_ten = '$ho_ten', ten_thanh = '$ten_thanh', ngay_sinh = '$ngay_sinh', gioi_tinh = '$gioi_tinh', ngay_rua_toi = '$ngay_rua_toi', ngay_them_suc = '$ngay_them_suc', ngay_gia_nhap = '$ngay_gia_nhap', so_dien_thoai = '$so_dien_thoai', email = '$email', dia_chi = '$dia_chi', ho_ten_bo = '$ho_ten_bo', ho_ten_me = '$ho_ten_me', sdt_lien_lac = '$sdt_lien_lac', email_phu_huynh = '$email_phu_huynh', ghi_chu = '$ghi_chu', loai_huynh = '$loai_huynh', updated_at = NOW() WHERE id = $teacher_id";
    } else {
        // Nếu chưa có các trường mới, chỉ cập nhật các trường cơ bản
        $sql_teacher = "UPDATE teachers SET ho_ten = '$ho_ten', ten_thanh = '$ten_thanh', ngay_sinh = '$ngay_sinh', so_dien_thoai = '$so_dien_thoai', email = '$email', dia_chi = '$dia_chi', ngay_gia_nhap = '$ngay_gia_nhap' WHERE id = $teacher_id";
    }
...
<body class="theme-default">
            ...
            <div class="card-body p-4">
                ...
                <?php if (!$has_loai_huynh): ?>
                    <div class="alert alert-warning">
                        ...
                        Vui lòng chạy file <code>update_teachers_table.sql</code> trong phpMyAdmin.
                    </div>
                <?php endif; ?>
                        ...
                        <tbody>
                            <?php $stt = 1; foreach ($hdt_list as $hdt): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge <?php echo $hdt['loai_huynh'] === 'Huynh Dự Trưởng' ? 'bg-warning' : 'bg-primary'; ?>">
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#editHDTModal" onclick="loadHDT(<?php echo $hdt['id']; ?>, '<?php echo addslashes(htmlspecialchars($hdt['ho_ten'])); ?>', '<?php echo addslashes(htmlspecialchars($hdt['ten_thanh'] ?? '')); ?>', '<?php echo $hdt['ngay_sinh'] ?? ''; ?>', '<?php echo htmlspecialchars($hdt['gioi_tinh'] ?? ''); ?>', '<?php echo $hdt['ngay_rua_toi'] ?? ''; ?>', '<?php echo $hdt['ngay_them_suc'] ?? ''; ?>', '<?php echo $hdt['ngay_gia_nhap'] ?? ''; ?>', '<?php echo addslashes(htmlspecialchars($hdt['so_dien_thoai'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['email'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['dia_chi'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['ho_ten_bo'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['ho_ten_me'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['sdt_lien_lac'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['email_phu_huynh'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['ghi_chu'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['vai_tro'] ?? '')); ?>', '<?php echo addslashes(htmlspecialchars($hdt['loai_huynh'] ?? '')); ?>', <?php echo $hdt['class_id']; ?>)">
                ...
                <div class="modal-body">
                    <form method="POST" id="editHDTForm">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Ngày Rửa Tội</label>
                                <input type="date" name="ngay_rua_toi" id="edit_hdt_ngay_rua_toi" class="form-control">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Ngày Thêm Sức</label>
                                <input type="date" name="ngay_them_suc" id="edit_hdt_ngay_them_suc" class="form-control">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Ngày Gia Nhập</label>
                                <input type="date" name="ngay_gia_nhap" id="edit_hdt_ngay_gia_nhap" class="form-control">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Lớp</label>
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Địa Chỉ</label>
                                <input type="text" name="dia_chi" id="edit_hdt_dia_chi" class="form-control">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Họ Tên Bố</label>
                                <input type="text" name="ho_ten_bo" id="edit_hdt_ho_ten_bo" class="form-control">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Họ Tên Mẹ</label>
                                <input type="text" name="ho_ten_me" id="edit_hdt_ho_ten_me" class="form-control">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">SĐT Liên Lạc</label>
                                <input type="text" name="sdt_lien_lac" id="edit_hdt_sdt_lien_lac" class="form-control" pattern="[0-9]{10}" maxlength="10" inputmode="numeric" title="SĐT phải gồm 10 số">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Email Phụ Huynh</label>
                                <input type="email" name="email_phu_huynh" id="edit_hdt_email_phu_huynh" class="form-control">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Loại Huynh</label>
                            ...
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Ghi Chú</label>
                                <input type="text" name="ghi_chu" id="edit_hdt_ghi_chu" class="form-control">
                            </div>
                        </div>
    ...
    <script>
        ...
        function loadHDT(id, ho_ten, ten_thanh, ngay_sinh, gioi_tinh, ngay_rua_toi, ngay_them_suc, ngay_gia_nhap, so_dien_thoai, email, dia_chi, ho_ten_bo, ho_ten_me, sdt_lien_lac, email_phu_huynh, ghi_chu, vai_tro, loai_huynh, class_id) {
            ...
            document.getElementById('edit_hdt_ngay_rua_toi').value = ngay_rua_toi || '';
            document.getElementById('edit_hdt_ngay_them_suc').value = ngay_them_suc || '';
            document.getElementById('edit_hdt_ngay_gia_nhap').value = ngay_gia_nhap || '';
            document.getElementById('edit_hdt_so_dien_thoai').value = so_dien_thoai || '';
            document.getElementById('edit_hdt_email').value = email || '';
            document.getElementById('edit_hdt_dia_chi').value = dia_chi || '';
            document.getElementById('edit_hdt_ho_ten_bo').value = ho_ten_bo || '';
            document.getElementById('edit_hdt_ho_ten_me').value = ho_ten_me || '';
            document.getElementById('edit_hdt_sdt_lien_lac').value = sdt_lien_lac || '';
            document.getElementById('edit_hdt_email_phu_huynh').value = email_phu_huynh || '';
            document.getElementById('edit_hdt_ghi_chu').value = ghi_chu || '';
        ...
        $(document).ready(function() {
            ...
            attachPhoneValidation('#addHDTForm', "input[name='sdt_lien_lac']");
            attachPhoneValidation('#editHDTForm', '#edit_hdt_so_dien_thoai');
            attachPhoneValidation('#editHDTForm', '#edit_hdt_sdt_lien_lac');
        });
    </script>
</body>
</html>
